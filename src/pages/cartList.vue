<template>
  <div class="cartList">
    <nav-header @logout="queryCartList" @login="queryCartList"></nav-header>
    <nav-bread>
      <span>My Cart</span>
    </nav-bread>
  ...
  <defs>
    ...
    <symbol id="icon-clock" viewBox="0 0 32 32">
                ...
                <li v-for="(item,index) in cartList" :key="index">
                  <div class="cart-tab-1">
                    <div class="cart-item-check">
                      <a href="javascipt:;" class="checkbox-btn item-check-btn" :class="{checked:item.checked=='1'}" @click="selectProduct(index)">
                    ...
                    <div class="item-quantity">
                      <div class="select-self select-self-open">
                        <div class="select-self-area">
                          <a class="input-sub" @click="minusItemNum(index)">-</a>
                          <span class="select-ipt">{{item.productNum}}</span>
                          <a class="input-add" @click="addItemNum(index)">+</a>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="cart-tab-5">
                    <div class="cart-item-opration">
                      <a href="javascript:;" class="item-edit-btn" @click="deleteItem(index,item.productId)">
            ...
            <div class="cart-foot-inner">
              <div class="cart-foot-l">
                <div class="item-all-check">
                  <a href="javascipt:;" @click="selctAll">
                        <span class="checkbox-btn item-check-btn" :class="{'checked':checkAll}">
      ...
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="removeItem">确认</a>
...
export default {
  ...
  methods: {
    ...
    addItemNum(idx) {//增加商品数量
      ...
      this.isLoading = true;
      let { productId, productNum } = this.cartList[idx];
      this.$http({
        method: "post",
        url: "api/v1/carts/changeProductNum.node",
        payload: {
          productId: productId,
    ...
    removeItem() {
      this.$http({
        method: "post",
        url: "api/v1/carts/removeProduct.node",
        payload: {
          productId: this.checkedItem.productId
        }
      })
        .then(res => {
          if (res.data.resultCode === "000000") {
    ...
    selectProduct(index) {
      ...
      if (this.cartList[index].checked == "1") {
        this.$set(this.cartList[index], "checked", "0");
        this.checkCount--;
      } else {
        this.$set(this.cartList[index], "checked", "1");
        this.checkCount++;
      }
      this.$http({
        method: "post",
        url: "api/v1/carts/selectProduct.node",
        payload: {
          productId: productId
        }
      });
    ...
    total() {
      ...
      for (let index = 0; index < this.cartList.length; index++) {
        ...
        if (element.checked == "1") {
          total += element.productNum * element.salePrice;
        }
      }
      return total;
    }
