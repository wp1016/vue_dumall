...
<div class="addressList">
    <nav-header @login="queryAddressList" @logout="queryAddressList"></nav-header>
    <nav-bread>
        <span>Address</span>
    </nav-bread>
  ...
  <div class="container">
    <div class="checkout-addr">
      ...
      <div class="addr-list-wrap">
        <div class="addr-list">
          <ul>
            <li v-for="(item,index) in addressListFilter" :key="index" :class="{'check':checkIndex==index}" @click="selectAddress(index)">
              <dl>
                <dt>{{item.userName}}</dt>
              ...
              <div class="addr-opration addr-del">
                <a href="javascript:;" class="addr-del-btn" @click="deleteAddress(item.addressId)">
            ...
            <li class="addr-new" @click="addBox=true">
        ...
        <div class="shipping-addr-more">
          <a class="addr-more-btn up-down-btn" href="javascript:;" v-if="this.addressList.length>3" @click="showAll" :class="{'open':limit>3}">
            more
            <i class="i-up-down">
      ...
      <div class="next-btn-wrap">
        <router-link class="btn btn--m btn--red" :to="{name:'orderConfirm',params:{addressId}}">Next</router-link>
      </div>
    </div>
  </div>
</div>
<Modal :mdShow="modalConfirm" @close="modalConfirm = false">
      ...
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="removeAddress">确认</a>
        <a class="btn btn--m btn--red" href="javascript:;" @click="modalConfirm = false">关闭</a>
      </div>
  </Modal>
<Modal :mdShow="addBox" @close="closeAddBox">
      <div slot="message">
        <div class="error-wrap">
            <span class="error error-show" v-show="errMsg">{{errMsg}}</span>
          </div>
          <ul>
              <li class="regi_form_input">
                <i class="icon IconPeople"></i>
                <input type="text" tabindex="1" name="userName" v-model="userName" placeholder="请输入收货人姓名" class="regi_login_input">
              </li>
              <li class="regi_form_input noMargin">
                  <i class="icon IconStreetName"></i>
                  <input type="text" tabindex="2" name="streetName" v-model="streetName" placeholder="请输入收货地址" class="regi_login_input">
              </li>
              <li class="regi_form_input noMargin">
                  <i class="icon IconPostCode"></i>
                  <input type="text" tabindex="3" name="postCode" v-model="postCode" placeholder="请输入邮政编码" class="regi_login_input">
              </li>
              <li class="regi_form_input noMargin">
                  <i class="icon IconTel"></i>
                  <input type="tel" tabindex="4" name="tel" v-model="tel" placeholder="请输入联系人手机号码" class="regi_login_input">
              </li>
          </ul>
      </div>
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="addAddress">保存</a>
        <a class="btn btn--m btn--red" href="javascript:;" @click="closeAddBox">取消</a>
      </div>
  </Modal>
...
export default {
  ...
  methods: {
    queryAddressList() {
      this.$http({
        method: "get",
        url: "api/v1/address/queryUserAddress.node"
      })
        .then(res => {
          if (res.data.resultCode === "000000") {
            ...
            this.$nextTick(() => {
              this.addressId = this.addressListFilter[
                this.checkIndex
              ].addressId;
            });
          }
    ...
    setDefault(addid) {
      this.$http({
        method: "post",
        url: "api/v1/address/setDefault.node",
        payload: {
          addId: addid
        }
      })
    ...
    addAddress() {
      if (this.checkAddress()) {
        this.$http({
          method: "post",
          url: "api/v1/address/addAddress.node",
          payload: {
    ...
    removeAddress() {
      this.$http({
        method: "post",
        url: "api/v1/address/removeAddress.node",
        payload: {
    ...
    selectAddress(idx) {
      this.checkIndex = idx;
      this.addressId = this.addressList[idx].addressId;
    },
    checkAddress() {
      if (!this.userName) {
        this.errMsg = "收货人姓名不能为空";
        return false;
      } else if (!this.streetName) {
        this.errMsg = "收货地址不能为空";
        return false;
      } else if (!this.tel) {
        this.errMsg = "收货人联系方式不能为空";
        return false;
      } else {
        this.errMsg = false;
        return true;
