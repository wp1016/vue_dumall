      ...
      <div class="navbar">
        ...
        <div class="navbar-right-container" style="display: flex;">
          <div class="navbar-menu-container">
            ...
            <a href="javascript:void(0)" class="navbar-link" v-show="!nickName" @click="loginModelFlag=true">Login</a>
...
export default {
  ...
  mounted() {
    this.$http({
      method: "get",
      url: "api/v1/users/verifyUserId.node"
    })
      .then(res => {
        if (res.data.resultCode === "000000") {
          this.$store.commit(
            "saveUserName",
            `welcome,${res.data.info.userName}`
          );
          this.getCartCount();
        } else {
  ...
  methods: {
    login() {
      if (!this.userName) {
        (this.errorTip = true), (this.errMsg = "用户名不能为空");
        return;
      }
      if (!this.userPwd) {
        (this.errorTip = true), (this.errMsg = "密码不能为空");
        return;
      }
      this.$http({
        method: "post",
        url: "api/v1/users/login.node",
        payload: {
    ...
    getCartCount() {
      this.$http({
        method: "get",
        url: "api/v1/carts/queryCartCount.node"
      })
        .then(res => {
          this.$store.commit("initCartCount", res.data.count);
        })
        .catch(err => {
          console.log(err);
        });
